...
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  KEY `idx_orders_lifecycle` (`payment_status`, `fulfillment_status`, `created_at`, `id`),
...
CREATE TABLE IF NOT EXISTS `tickets` (
  ...
  KEY `idx_tickets_order` (`order_id`, `id`),
  KEY `idx_tickets_voided` (`voided_at`, `id`),
  ...
  CONSTRAINT `fk_tickets_replaced_by` FOREIGN KEY (`replaced_by_ticket_id`) REFERENCES `tickets` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Durable Google Wallet Generic Object synchronization outbox
...
-- Durable cleanup queue for private files removed with database records
