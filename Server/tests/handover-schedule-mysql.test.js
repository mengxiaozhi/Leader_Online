...
const { STAGES, updateHandoverSchedule, syncHandoverRecipients, scheduleFromRow } = require('../src/services/handover-schedule');
...
test('handover migration, rollback, reminders and transfers on real MySQL', { skip: !socketPath }, async t => {
  ...
  const save = async rollback => {
    ...
    try {
      ...
      await updateHandoverSchedule(conn, { store, stages, expectedVersion: 1, now });
