    ...
    <section class="space-y-4" :aria-labelledby="`course-admin-${adminTask.key}`">
        ...
        <template v-else-if="adminTask.key === 'classes'">
          ...
          <template v-if="productizedClassSection === 'terms'">
            ...
            <AdminPagination detailed v-if="productizedFilteredItems.length" v-bind="productizedPagination" :loading="productizedLoading" @change="changeProductizedPage" />
          </template>
        </template>
        ...
        <template v-else>
          <div v-if="!productizedItems.length" class="surface-section text-sm leading-6 text-slate-600">{{ productizedEmptyText }}</div>
          <div v-else class="overflow-hidden rounded-xl border border-slate-200 bg-white"><AdminTableFrame class="hidden md:block" label="課程紀錄" :count="productizedItems.length"><table class="admin-data-table table-default " aria-label="課程紀錄" style="--admin-table-width: 72rem">
<colgroup><col  style="width: 19rem" /><col  style="width: 10rem" /><col  style="width: 13rem" /><col  style="width: 16rem" /><col  style="width: 14rem" /></colgroup><thead><tr><th scope="col" class="admin-table-identity">名稱／編號</th><th scope="col">狀態</th><th scope="col">名額／權益</th><th scope="col">時間</th><th scope="col" class="admin-table-actions">下一步</th></tr></thead><tbody><tr v-for="item in productizedItems" :key="item.id || item.code"><td class="admin-table-identity"><strong class="text-slate-900">{{ item.name || item.title || item.termName || item.term_name || item.studentName || item.student_name || '課程紀錄' }}</strong><p class="text-xs text-slate-500">{{ item.code || item.email || item.sessionCode || item.session_code || '—' }}</p></td><td><span class="ops-chip" :class="adminStatusClass(item)">{{ adminStatusLabel(item) }}</span></td><td>{{ adminCapacityLabel(item) }}</td><td>{{ formatRange(item.startsAt || item.starts_at, item.endsAt || item.ends_at) }}</td><td class="admin-table-actions"><router-link v-if="item.orderId || item.order_id" :to="courseRecordDeepLink('orders', item.orderId || item.order_id)" class="text-sm font-medium text-primary">查看課程訂單</router-link><span v-else class="text-sm text-slate-500">依伺服器 capability 開放操作</span></td></tr></tbody></table></AdminTableFrame>
            <div class="grid gap-3 p-3 md:hidden">
              <article v-for="item in productizedItems" :key="item.id || item.code" class="min-w-0 rounded-lg border border-slate-200 bg-white p-4">
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0 break-words"><h3 class="font-medium text-slate-950">{{ item.name || item.title || item.termName || item.term_name || item.studentName || item.student_name || '課程紀錄' }}</h3><p class="mt-1 break-all text-xs text-slate-500">{{ item.code || item.email || item.sessionCode || item.session_code || '—' }}</p></div>
                  <span class="ops-chip shrink-0" :class="adminStatusClass(item)">{{ adminStatusLabel(item) }}</span>
                </div>
                <dl class="mt-3 grid gap-3 text-sm"><div><dt class="text-xs text-slate-500">名額／權益</dt><dd class="mt-1">{{ adminCapacityLabel(item) }}</dd></div><div><dt class="text-xs text-slate-500">時間</dt><dd class="mt-1">{{ formatRange(item.startsAt || item.starts_at, item.endsAt || item.ends_at) }}</dd></div></dl>
                <router-link v-if="item.orderId || item.order_id" :to="courseRecordDeepLink('orders', item.orderId || item.order_id)" class="btn btn-outline mt-3 w-full">查看課程訂單</router-link>
              </article>
            </div></div>
        </template>
        </CourseResourceState>
  ...
  <section v-else class="space-y-5">
    ...
    <section v-else-if="activeTab === 'products'" class="space-y-4">
      ...
      <AdminTableState v-else :loading="loading.products" :empty="!products.length" :empty-text="emptyText('products', '尚無課程商品。')">
        <AdminTableFrame class="hidden md:block" label="課程商品" :count="products.length">
          <table class="admin-data-table table-default " aria-label="課程商品" :style="{ '--admin-table-width': isAdmin ? '110rem' : '95rem' }">
<colgroup><col  style="width: 19rem" /><col  style="width: 9rem" /><col v-if="isAdmin" style="width: 15rem" /><col  style="width: 10rem" /><col  style="width: 13rem" /><col  style="width: 8rem" /><col  style="width: 9rem" /><col  style="width: 12rem" /><col  style="width: 15rem" /></colgroup>
            <thead><tr>
              <th scope="col" class="admin-table-identity">代碼／名稱</th>
              <th scope="col"><TableColumnFilter mode="server" label="分類" :fields="columnFields(productFilterColumns, 'category')" :model-value="filters.products.category" @update:model-value="setColumnFilter('products', 'category', $event)" @apply="applyColumnFilter('products', 'category', $event)" /></th>
              <th scope="col" v-if="isAdmin"><TableColumnFilter mode="server" label="服務商" :fields="columnFields(productFilterColumns, 'provider')" :model-value="filters.products.provider" @update:model-value="setColumnFilter('products', 'provider', $event)" @apply="applyColumnFilter('products', 'provider', $event)" /></th>
              <th class="admin-table-number" scope="col"><TableColumnFilter mode="server" label="價格" :fields="columnFields(productFilterColumns, 'price')" :model-value="filters.products.price" @update:model-value="setColumnFilter('products', 'price', $event)" @apply="applyColumnFilter('products', 'price', $event)" /></th>
              <th scope="col"><TableColumnFilter mode="server" label="堂數／效期" :fields="columnFields(productFilterColumns, 'usage')" :model-value="filters.products.usage" @update:model-value="setColumnFilter('products', 'usage', $event)" @apply="applyColumnFilter('products', 'usage', $event)" /></th>
              <th scope="col"><TableColumnFilter mode="server" label="轉讓" :fields="columnFields(productFilterColumns, 'transfer')" :model-value="filters.products.transfer" @update:model-value="setColumnFilter('products', 'transfer', $event)" @apply="applyColumnFilter('products', 'transfer', $event)" /></th>
              <th scope="col"><TableColumnFilter mode="server" label="狀態" :fields="columnFields(productFilterColumns, 'status')" :model-value="filters.products.status" @update:model-value="setColumnFilter('products', 'status', $event)" @apply="applyColumnFilter('products', 'status', $event)" /></th>
              <th scope="col"><TableColumnFilter mode="server" label="更新日期" :fields="columnFields(productFilterColumns, 'updated')" :model-value="filters.products.updated" @update:model-value="setColumnFilter('products', 'updated', $event)" @apply="applyColumnFilter('products', 'updated', $event)" /></th>
              <th scope="col" class="admin-table-actions">操作</th>
            </tr></thead>
            <tbody>
              <tr v-for="product in products" :key="product.id">
                <td class="admin-table-identity"><p class="font-medium text-slate-900">{{ product.name }}</p><p class="text-sm text-slate-500">{{ product.code }}</p></td>
                ...
                <td class="admin-table-number money-value">NT$ {{ formatMoney(product.price) }}</td>
                ...
                <td class="admin-table-actions"><div class="flex gap-2"><button type="button" class="btn btn-outline btn-sm" @click="openProductForm(product)"><AppIcon name="edit" class="h-4 w-4" /> 編輯</button><button v-if="product.status !== 'archived'" type="button" class="btn btn-outline btn-sm text-red-700" @click="archiveProduct(product)">封存</button></div></td>
              </tr>
            </tbody>
          </table>
        </AdminTableFrame>
      ...
      <AdminPagination detailed v-if="!errors.products" v-bind="meta.products" :loading="loading.products" @change="changePage('products', $event)" />
    </section>
    ...
    <section v-else-if="activeTab === 'sessions'" class="space-y-4">
      ...
      <AdminTableState v-else :loading="loading.sessions" :empty="!sessions.length" :empty-text="emptyText('sessions', '尚無課程場次。')">
        <AdminTableFrame class="hidden md:block" label="課程場次" :count="sessions.length"><table class="admin-data-table table-default " aria-label="課程場次" :style="{ '--admin-table-width': isAdmin ? '120rem' : '105rem' }">
<colgroup><col  style="width: 19rem" /><col  style="width: 17rem" /><col v-if="isAdmin" style="width: 15rem" /><col  style="width: 16rem" /><col  style="width: 18rem" /><col  style="width: 9rem" /><col  style="width: 10rem" /><col  style="width: 16rem" /></colgroup><thead><tr>
          <th scope="col" class="admin-table-identity">場次</th><th scope="col"><TableColumnFilter mode="server" label="適用商品" :fields="columnFields(sessionFilterColumns, 'product')" :model-value="filters.sessions.product" @update:model-value="setColumnFilter('sessions', 'product', $event)" @apply="applyColumnFilter('sessions', 'product', $event)" /></th>
          <th scope="col" v-if="isAdmin"><TableColumnFilter mode="server" label="服務商" :fields="columnFields(sessionFilterColumns, 'provider')" :model-value="filters.sessions.provider" @update:model-value="setColumnFilter('sessions', 'provider', $event)" @apply="applyColumnFilter('sessions', 'provider', $event)" /></th>
          <th scope="col"><TableColumnFilter mode="server" label="時間" :fields="columnFields(sessionFilterColumns, 'time')" :model-value="filters.sessions.time" @update:model-value="setColumnFilter('sessions', 'time', $event)" @apply="applyColumnFilter('sessions', 'time', $event)" /></th>
          <th scope="col"><TableColumnFilter mode="server" label="教練／地點" :fields="columnFields(sessionFilterColumns, 'place')" :model-value="filters.sessions.place" @update:model-value="setColumnFilter('sessions', 'place', $event)" @apply="applyColumnFilter('sessions', 'place', $event)" /></th>
          <th class="admin-table-number" scope="col"><TableColumnFilter mode="server" label="名額" :fields="columnFields(sessionFilterColumns, 'availability')" :model-value="filters.sessions.availability" @update:model-value="setColumnFilter('sessions', 'availability', $event)" @apply="applyColumnFilter('sessions', 'availability', $event)" /></th>
          <th scope="col"><TableColumnFilter mode="server" label="狀態" :fields="columnFields(sessionFilterColumns, 'status')" :model-value="filters.sessions.status" @update:model-value="setColumnFilter('sessions', 'status', $event)" @apply="applyColumnFilter('sessions', 'status', $event)" /></th><th scope="col" class="admin-table-actions">操作</th>
        </tr></thead><tbody><tr v-for="session in sessions" :key="session.id">
          <td class="admin-table-identity"><p class="font-medium text-slate-900">{{ session.title }}</p><p class="text-sm text-slate-500">{{ session.code }}</p></td><td>{{ session.productName || '同服務商全部課程票券' }}</td><td v-if="isAdmin">{{ providerDisplay(session) }}</td><td>{{ formatRange(session.startsAt, session.endsAt) }}</td><td><p>{{ session.coachName || '教練待公告' }}</p><p class="text-sm text-slate-500">{{ session.location || '地點待公告' }}</p></td><td class="admin-table-number">{{ session.bookedCount }}/{{ session.capacity }}</td><td><span class="ops-chip" :class="statusChip(session.status)">{{ sessionStatusLabel(session.status) }}</span></td><td class="admin-table-actions"><div class="flex gap-2"><button type="button" class="btn btn-outline btn-sm" @click="openSessionForm(session)">編輯</button><button v-if="session.status !== 'cancelled'" type="button" class="btn btn-outline btn-sm text-red-700" @click="cancelSession(session)">取消</button></div></td>
        </tr></tbody></table></AdminTableFrame>
      ...
      <AdminPagination detailed v-if="!errors.sessions" v-bind="meta.sessions" :loading="loading.sessions" @change="changePage('sessions', $event)" />
    </section>
    ...
    <section v-else-if="activeTab === 'orders'" class="space-y-4">
      ...
      <AdminTableState v-else :loading="loading.orders" :empty="!orders.length" :empty-text="emptyText('orders', '尚無課程訂單。')">
        <AdminTableFrame class="hidden md:block" label="課程訂單" :count="orders.length"><table class="admin-data-table table-default " aria-label="課程訂單" :style="{ '--admin-table-width': isAdmin ? '128rem' : '113rem' }">
<colgroup><col  style="width: 3rem" /><col  style="width: 14rem" /><col  style="width: 18rem" /><col  style="width: 18rem" /><col v-if="isAdmin" style="width: 15rem" /><col  style="width: 14rem" /><col  style="width: 8rem" /><col  style="width: 10rem" /><col  style="width: 12rem" /><col  style="width: 16rem" /></colgroup><thead><tr><th scope="col"><input type="checkbox" :checked="allVisibleOrdersSelected" :indeterminate="orders.some(isOrderSelected) && !allVisibleOrdersSelected" :aria-label="allVisibleOrdersSelected ? '取消選取目前頁訂單' : '選取目前頁訂單'" @change="toggleAllVisibleOrders($event.target.checked)" /></th><th scope="col" class="admin-table-identity">訂單</th><th scope="col"><TableColumnFilter mode="server" label="購買人" :fields="columnFields(orderFilterColumns, 'user')" :model-value="filters.orders.user" @update:model-value="setColumnFilter('orders', 'user', $event)" @apply="applyColumnFilter('orders', 'user', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="課程" :fields="columnFields(orderFilterColumns, 'product')" :model-value="filters.orders.product" @update:model-value="setColumnFilter('orders', 'product', $event)" @apply="applyColumnFilter('orders', 'product', $event)" /></th><th scope="col" v-if="isAdmin"><TableColumnFilter mode="server" label="服務商" :fields="columnFields(orderFilterColumns, 'provider')" :model-value="filters.orders.provider" @update:model-value="setColumnFilter('orders', 'provider', $event)" @apply="applyColumnFilter('orders', 'provider', $event)" /></th><th class="admin-table-number" scope="col"><TableColumnFilter mode="server" label="數量／金額" :fields="columnFields(orderFilterColumns, 'amount')" :model-value="filters.orders.amount" @update:model-value="setColumnFilter('orders', 'amount', $event)" @apply="applyColumnFilter('orders', 'amount', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="後五碼" :fields="columnFields(orderFilterColumns, 'remittance')" :model-value="filters.orders.remittance" @update:model-value="setColumnFilter('orders', 'remittance', $event)" @apply="applyColumnFilter('orders', 'remittance', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="狀態" :fields="columnFields(orderFilterColumns, 'status')" :model-value="filters.orders.status" @update:model-value="setColumnFilter('orders', 'status', $event)" @apply="applyColumnFilter('orders', 'status', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="建立時間" :fields="columnFields(orderFilterColumns, 'created')" :model-value="filters.orders.created" @update:model-value="setColumnFilter('orders', 'created', $event)" @apply="applyColumnFilter('orders', 'created', $event)" /></th><th scope="col" class="admin-table-actions">操作</th></tr></thead><tbody><tr v-for="order in orders" :key="order.id" :class="{ 'admin-table-row-selected': isOrderSelected(order) }"><td><input type="checkbox" :checked="isOrderSelected(order)" :aria-label="`選取訂單 ${order.code}`" @change="toggleOrder(order, $event.target.checked)" /></td><td class="admin-table-identity font-medium text-slate-900">{{ order.code }}</td><td><p>{{ order.buyerName }}</p><p class="text-sm text-slate-500">{{ order.buyerEmail }}</p></td><td>{{ order.productName }}</td><td v-if="isAdmin">{{ providerDisplay(order) }}</td><td class="admin-table-number">{{ order.quantity }} 份／<span class="money-value">NT$ {{ formatMoney(order.totalAmount) }}</span></td><td>{{ order.remittanceLast5 || '—' }}</td><td><span class="ops-chip" :class="orderStatusClass(order)">{{ orderStatusLabel(order) }}</span></td><td>{{ formatDateTime(order.createdAt) }}</td><td class="admin-table-actions"><div class="flex gap-2"><button type="button" class="btn btn-outline btn-sm" @click="openOrderDetail(order)">詳情</button><button v-if="hasOrderCapability(order, 'editPricing')" type="button" class="btn btn-outline btn-sm" :disabled="priceEditor.saving" @click="openPriceEditor(order)">修改金額</button><button v-if="primaryOrderAction(order)" type="button" class="btn btn-primary btn-sm text-white" :disabled="busyId === `order-${order.id}`" @click="runOrderAction(order, primaryOrderAction(order).value)">{{ primaryOrderAction(order).label }}</button></div></td></tr></tbody></table></AdminTableFrame>
      ...
      <AdminPagination detailed v-if="!errors.orders" v-bind="meta.orders" :loading="loading.orders" @change="changePage('orders', $event)" />
    </section>
    ...
    <section v-else-if="activeTab === 'tickets'" class="space-y-4">
      ...
      <AdminTableState v-else :loading="loading.tickets" :empty="!tickets.length" :empty-text="emptyText('tickets', '尚無課程票券。')">
        <AdminTableFrame class="hidden md:block" label="課程票券" :count="tickets.length"><table class="admin-data-table table-default " aria-label="課程票券" :style="{ '--admin-table-width': isAdmin ? '126rem' : '111rem' }">
<colgroup><col  style="width: 16rem" /><col  style="width: 18rem" /><col  style="width: 18rem" /><col v-if="isAdmin" style="width: 15rem" /><col  style="width: 12rem" /><col  style="width: 12rem" /><col  style="width: 12rem" /><col  style="width: 10rem" /><col  style="width: 13rem" /></colgroup><thead><tr><th scope="col" class="admin-table-identity">票券</th><th scope="col"><TableColumnFilter mode="server" label="持有人" :fields="columnFields(ticketFilterColumns, 'holder')" :model-value="filters.tickets.holder" @update:model-value="setColumnFilter('tickets', 'holder', $event)" @apply="applyColumnFilter('tickets', 'holder', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="商品" :fields="columnFields(ticketFilterColumns, 'product')" :model-value="filters.tickets.product" @update:model-value="setColumnFilter('tickets', 'product', $event)" @apply="applyColumnFilter('tickets', 'product', $event)" /></th><th scope="col" v-if="isAdmin"><TableColumnFilter mode="server" label="服務商" :fields="columnFields(ticketFilterColumns, 'provider')" :model-value="filters.tickets.provider" @update:model-value="setColumnFilter('tickets', 'provider', $event)" @apply="applyColumnFilter('tickets', 'provider', $event)" /></th><th class="admin-table-number" scope="col"><TableColumnFilter mode="server" label="剩餘／總堂數" :fields="columnFields(ticketFilterColumns, 'usage')" :model-value="filters.tickets.usage" @update:model-value="setColumnFilter('tickets', 'usage', $event)" @apply="applyColumnFilter('tickets', 'usage', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="發行時間" :fields="columnFields(ticketFilterColumns, 'created')" :model-value="filters.tickets.created" @update:model-value="setColumnFilter('tickets', 'created', $event)" @apply="applyColumnFilter('tickets', 'created', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="效期" :fields="columnFields(ticketFilterColumns, 'expiry')" :model-value="filters.tickets.expiry" @update:model-value="setColumnFilter('tickets', 'expiry', $event)" @apply="applyColumnFilter('tickets', 'expiry', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="狀態" :fields="columnFields(ticketFilterColumns, 'status')" :model-value="filters.tickets.status" @update:model-value="setColumnFilter('tickets', 'status', $event)" @apply="applyColumnFilter('tickets', 'status', $event)" /></th><th scope="col" class="admin-table-actions">操作</th></tr></thead><tbody><tr v-for="ticket in tickets" :key="ticket.id"><td class="admin-table-identity font-medium text-slate-900">{{ ticket.code }}</td><td><p>{{ ticket.ownerName || '—' }}</p><p class="text-sm text-slate-500">{{ ticket.ownerEmail }}</p></td><td>{{ ticket.productName }}</td><td v-if="isAdmin">{{ providerDisplay(ticket) }}</td><td class="admin-table-number">{{ ticket.remainingUses }}／{{ ticket.totalUses }}</td><td>{{ formatDateTime(ticket.createdAt) }}</td><td>{{ ticket.expiresAt || '未啟用' }}</td><td><span class="ops-chip" :class="statusChip(ticket.status)">{{ ticketStatusLabel(ticket.status) }}</span></td><td class="admin-table-actions"><button type="button" class="btn btn-outline btn-sm" @click="openTicketDetail(ticket)">詳情與紀錄</button></td></tr></tbody></table></AdminTableFrame>
      ...
      <AdminPagination detailed v-if="!errors.tickets" v-bind="meta.tickets" :loading="loading.tickets" @change="changePage('tickets', $event)" />
    </section>
...
<colgroup><col  style="width: 19rem" /><col  style="width: 18rem" /><col  style="width: 21rem" /><col v-if="isAdmin" style="width: 15rem" /><col  style="width: 12rem" /><col  style="width: 16rem" /><col  style="width: 10rem" /><col  style="width: 13rem" /></colgroup><thead><tr><th scope="col" class="admin-table-identity"><TableColumnFilter mode="server" label="場次" :fields="columnFields(bookingFilterColumns, 'session')" :model-value="filters.bookings.session" @update:model-value="setColumnFilter('bookings', 'session', $event)" @apply="applyColumnFilter('bookings', 'session', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="學員" :fields="columnFields(bookingFilterColumns, 'user')" :model-value="filters.bookings.user" @update:model-value="setColumnFilter('bookings', 'user', $event)" @apply="applyColumnFilter('bookings', 'user', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="票券／商品" :fields="columnFields(bookingFilterColumns, 'ticket')" :model-value="filters.bookings.ticket" @update:model-value="setColumnFilter('bookings', 'ticket', $event)" @apply="applyColumnFilter('bookings', 'ticket', $event)" /></th><th scope="col" v-if="isAdmin"><TableColumnFilter mode="server" label="服務商" :fields="columnFields(bookingFilterColumns, 'provider')" :model-value="filters.bookings.provider" @update:model-value="setColumnFilter('bookings', 'provider', $event)" @apply="applyColumnFilter('bookings', 'provider', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="預約時間" :fields="columnFields(bookingFilterColumns, 'booked')" :model-value="filters.bookings.booked" @update:model-value="setColumnFilter('bookings', 'booked', $event)" @apply="applyColumnFilter('bookings', 'booked', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="場次時間" :fields="columnFields(bookingFilterColumns, 'starts')" :model-value="filters.bookings.starts" @update:model-value="setColumnFilter('bookings', 'starts', $event)" @apply="applyColumnFilter('bookings', 'starts', $event)" /></th><th scope="col"><TableColumnFilter mode="server" label="狀態" :fields="columnFields(bookingFilterColumns, 'status')" :model-value="filters.bookings.status" @update:model-value="setColumnFilter('bookings', 'status', $event)" @apply="applyColumnFilter('bookings', 'status', $event)" /></th><th scope="col" class="admin-table-actions">操作</th></tr></thead><tbody><tr v-for="booking in bookings" :key="booking.id"><td class="admin-table-identity"><p class="font-medium text-slate-900">{{ booking.sessionTitle }}</p><p class="text-sm text-slate-500">{{ booking.sessionCode }}</p></td><td><p>{{ booking.attendeeName }}</p><p class="text-sm text-slate-500">{{ booking.attendeeEmail }}</p></td><td><p>{{ booking.ticketCode }}</p><p class="text-sm text-slate-500">{{ booking.productName || '' }}・剩餘 {{ booking.remainingUses }}・保留 {{ booking.heldUses ?? 0 }}・可用 {{ booking.availableUses ?? booking.remainingUses }} 堂</p></td><td v-if="isAdmin">{{ providerDisplay(booking) }}</td><td>{{ formatDateTime(booking.bookedAt || booking.createdAt) }}</td><td><p>{{ formatDateTime(booking.startsAt) }}</p><p class="text-sm text-slate-500">{{ booking.location || '地點待公告' }}</p></td><td><span class="ops-chip" :class="statusChip(booking.status)">{{ bookingStatusLabel(booking.status) }}</span></td><td class="admin-table-actions"><button type="button" class="btn btn-outline btn-sm" @click="openBookingDetail(booking)">{{ booking.status === 'booked' ? '課務操作' : '詳情' }}</button></td></tr></tbody></table></AdminTableFrame>
      ...
      <AdminPagination detailed v-if="!errors.bookings" v-bind="meta.bookings" :loading="loading.bookings" @change="changePage('bookings', $event)" />
    </section>
