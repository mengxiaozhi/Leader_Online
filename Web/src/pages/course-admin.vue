<template>
  <section class="space-y-5">
    ...
    <section v-else-if="activeTab === 'orders'" class="space-y-4">
      ...
      <p v-if="!bulkOrderMutationEnabled" class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        Course V2 需要逐筆驗證訂單版本，批次狀態更新已停用；請開啟各筆訂單逐一更新。
      </p>
...
const defaultCourseOwnerUserId = computed(() => {
  ...
  const membershipOwner = (props.memberships || [])
    .map(item => String(item?.ownerUserId ?? item?.owner_user_id ?? '').trim())
    .find(Boolean)
  if (membershipOwner) return membershipOwner
  return role.value === 'SERVICE_PROVIDER' ? String(props.currentUserId || '').trim() : ''
})
...
const emptyProductForm = () => ({ ownerUserId: '', name: '', ticketProductId: '', requireAddonForNew: false, returningProductIds: [], requiredAddonProductIds: [], category: '', summary: '', description: '', coverUrl: '', externalPurchaseUrl: '', hasCover: false, price: 0, classCount: 1, validDays: 120, activationDays: 120, transferable: false, status: 'draft', sortOrder: 0, rowVersion: '' })
...
const bulkOrderMutationEnabled = computed(() => !props.courseV2Enabled)
...
async function bulkUpdateOrders() {
  if (!bulkOrderMutationEnabled.value) {
    clearOrderSelection()
    showMessage('Course V2 需要逐筆驗證訂單版本，請逐筆更新訂單狀態。', 'error')
    return
  }
  if (!selectedOrderIds.value.length || !bulkOrderStatus.value || bulkOrderStatus.value === 'issued') return
  if (!window.confirm(`確定批次更新 ${selectedOrderIds.value.length} 筆訂單狀態？`)) return
  bulkSaving.value = true
  try {
    await axios.patch(
      `${API}/admin/courses/orders/bulk`,
      { ids: selectedOrderIds.value, status: bulkOrderStatus.value },
      courseMutationConfig({}, 'course-order-bulk-update')
    )
    clearOrderSelection()
    await Promise.all([loadList('orders', { force: true }), loadOverview()])
    showMessage('課程訂單已批次更新。')
  } catch (error) {
    if (!await reloadCourseMutationConflict(error, 'orders', {
      message: '部分訂單狀態已更新，清單已重新載入，請重新選取。',
    })) showMessage(error?.response?.data?.message || '批次更新失敗', 'error')
  } finally { bulkSaving.value = false }
}
async function issueOrder(order) {
  if (!window.confirm(`確認依訂單 ${order.code} 發行 ${order.quantity} 張課程票券？`)) return
  busyId.value = `order-${order.id}`
  detailSaving.value = true
  try {
    await axios.post(
      `${API}/admin/courses/orders/${order.id}/issue`,
      {},
      courseMutationConfig(order, 'course-order-issue')
    )
    detailOpen.value = false
    await Promise.all([loadList('orders', { force: true }), loadOverview()])
    showMessage(`訂單 ${order.code} 已完成發券。`)
  } catch (error) {
    if (!await reloadCourseMutationConflict(error, 'orders', {
      closeDetails: true,
      message: '課程訂單版本已更新，清單已重新載入，請重新確認後再發券。',
    })) showMessage(error?.response?.data?.message || '訂單發券失敗', 'error')
...
async function saveOrderDetail() {
  ...
  try {
    await axios.patch(
      `${API}/admin/courses/orders/${detailRecord.value.id}`,
      { status: detailRecord.value.status, note: detailRecord.value.note || '' },
      courseMutationConfig(detailRecord.value, 'course-order-update')
    )
    detailOpen.value = false
    await Promise.all([loadList('orders', { force: true }), loadOverview()])
    showMessage('課程訂單已更新。')
    ...
    if (!await reloadCourseMutationConflict(error, 'orders', {
      closeDetails: true,
      message: '課程訂單版本已更新，清單已重新載入，請重新開啟後再更新。',
    })) showMessage(error?.response?.data?.message || '訂單更新失敗', 'error')
...
async function issueManualTicket() {
  ...
  try {
    const { data } = await axios.post(
      ...
      { ...ticketForm.value, productId: Number(ticketForm.value.productId) },
...
async function runBookingAction(booking, action) {
  ...
  detailSaving.value = true
  try {
    await axios.post(`${API}/admin/courses/bookings/${booking.id}/${definition.endpoint}`, {}, courseMutationConfig(booking, `booking-${definition.endpoint}`))
    detailOpen.value = false
