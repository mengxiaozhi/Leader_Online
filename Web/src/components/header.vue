    ...
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 py-3 md:px-2">
      ...
      <nav class="hidden items-center gap-1 text-[0.95rem] font-medium md:flex" aria-label="主要導覽">
        <router-link v-for="item in navMenu" :key="item.id || item.path" :to="item.path"
          class="interactive-press flex min-h-11 items-center gap-2 border-b-2 px-3 py-2 transition"
          :class="isActiveItem(item) ? 'border-primary text-primary' : 'border-transparent text-slate-700 hover:text-primary'"
        ...
        <router-link v-if="isBrandPage" to="/store"
          class="interactive-press ml-2 flex min-h-11 items-center rounded-lg border border-primary bg-primary px-4 py-2 text-white transition hover:bg-secondary">
          前往商店
        </router-link>
        <router-link v-else-if="!isAuthed" to="/login"
          class="interactive-press ml-2 flex min-h-11 items-center gap-2 rounded-lg border border-primary bg-primary px-3 py-2 text-white transition hover:bg-secondary"
