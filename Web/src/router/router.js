...
const routes = [
    { path: '/', redirect: '/store' },
    { name: '登入', path: '/login', component: () => import('../pages/login.vue'), meta: { seo: { title: '登入', description: '登入 Leader Online，管理票券、預約貨車服務並查看最新訂單狀態。' } } },
    { name: '票券', path: '/wallet', component: () => import('../pages/wallet.vue'), meta: { requiresAuth: true, keepAlive: true, seo: { title: '我的票券', description: '查看已購買的貨車托運票券、預約紀錄與票券使用狀態。' } } },
    { name: '商店', path: '/store', component: () => import('../pages/store.vue'), meta: { keepAlive: true, seo: { title: '貨車托運購票中心', description: '選購貨車托運票券、同步雲端購物車並完成服務預約。' } } },
